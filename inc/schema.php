 ...
 *
 * Three pieces, one class per file (inc/schema/), same convention as WordPress
 ...
 *   Weizenkorn_Schema_LocalBusiness  schema.org/LocalBusiness (or a subtype — Bakery,
 *                                    Restaurant, Hotel, CafeOrCoffeeShop) for the
 *                                    Gastronomie venues, one node per physical location —
 *                                    see schema-gastronomie-acf-fields.md (theme root) for
 *                                    the ACF fields it reads.
 *
 * All three classes extend Yoast's own Abstract_Schema_Piece, so this file does nothing
 * (no fatal error, the class files are never required) if Yoast SEO is ever deactivated.
 *
 * @package weizenkorn
...
require get_template_directory() . '/inc/schema/class-weizenkorn-schema-localbusiness.php';

/**
 * Registers all custom pieces with Yoast's schema graph.
 *
...
function weizenkorn_add_schema_pieces( $pieces ) {
	...
	$pieces[] = new Weizenkorn_Schema_LocalBusiness();
