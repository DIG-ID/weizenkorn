...
### Added
- The login screen shows the site's own logo — whichever one is set under Appearance → Customize → Site Identity, the same one the header uses. Setting it there is all it takes; sites that set none keep the starter's mark.
...
- Sharing a page on social media shows that page's own hero picture, at its real size. The theme keeps its heroes in custom fields, which the SEO plugin cannot see, so every page used to share the same default image — and then, briefly, the right picture described by the wrong dimensions, which is what a network reads before it decides how to crop the card. Pages that draw no hero — the three legal pages and the job postings — still fall back to the default.
...
- Mega menu: on a wide (2K/4K) screen the photo no longer stretches tall — its rows no longer grow past the height the menu links actually need.
...
- The "Über die Händlerplattform pepperi" reseller link no longer opens anything, on the products archive and every product range page alike — it reads as plain text, with no arrow.
...
- The sticky promo box now shows on mobile too, tucked into the corner (20px from the right, 35px from the bottom) at a smaller 80px collapsed size, a narrower 220px box, 14px text and a smaller arrow, and starts collapsed there instead of waiting a few seconds; tablet and desktop are unchanged.
...
- Job postings and news posts carry a properly cut meta description. A posting had none at all — its text lives in a custom field the SEO plugin cannot read — and a news post had one that stopped mid-sentence with nothing to show it had been cut. Both now end on a whole word with an ellipsis, and a description written by hand still wins.
...
- Display headings are set tighter below desktop; from desktop up both keep the letter-spacing they had.
